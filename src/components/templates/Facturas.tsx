...
const Facturas = () => {
  ...
  console.log('Loading municipios:', loadingMunicipios);
  ...
  if (viewingNumber && facturaDetalleResponse && !loadingDetalle) {
    console.log('Datos de factura cargados:', facturaDetalleResponse);

    const bill = facturaDetalleResponse.bill;

    if (bill) {
      console.log('Bill encontrado:', bill);

      if (bill.public_url) {
        console.log('Abriendo URL pública:', bill.public_url);
        window.open(bill.public_url, '_blank', 'noopener,noreferrer');
        showSuccessToast('Abriendo factura en nueva pestaña');
      } else {
        console.log('Factura sin URL pública:', bill);
        showErrorToast('La factura no tiene URL pública disponible');
      }
    } else {
      console.log('No se encontró el objeto bill:', facturaDetalleResponse);
  ...
  useEffect(() => {
    console.log('useEffect descarga - downloadingNumber:', downloadingNumber);
    console.log('useEffect descarga - facturaDescarga:', facturaDescarga);
    console.log('useEffect descarga - loadingDescarga:', loadingDescarga);
    ...
    if (downloadingNumber && facturaDescarga) {
      ...
      if (base64 && fileName) {
        ...
        try {
          ...
          console.log('Longitud del base64:', base64.length);
    ...
    { key: "total", label: "Total", filterable: true },
    {
      key: "acciones", label: "Acciones", render: (factura) => (
        <div className="flex gap-2">
          <Botton onClick={() => handleView(factura)} className="bg-[#8b4646] text-white">Ver</Botton>
  ...
  const formFieldsCreate: FormField[] = [
    ...
    { key: "observation", label: "Observación", type: "text", required: false },
    { key: "payment_method_code", label: "Método de Pago", type: "text", required: false },
    ...
    { key: "item_code_reference", label: "Código del Producto", type: "text", required: true },
    { key: "item_name", label: "Nombre del Producto", type: "text", required: true },
    { key: "item_quantity", label: "Cantidad", type: "number", required: true },
    { key: "item_price", label: "Precio Unitario", type: "number", required: true },
    { key: "item_tax_rate", label: "Tasa de Impuesto (%)", type: "text", required: true },
  ];
  ...
  const handleSubmit = async (values: Record<string, string>) => {
    try {
      if (editingId) {
        ...
        // Crear nueva factura con la estructura correcta según la documentación
        const createPayload: FacturaCreateRequest = {
          ...
          items: [
            {
              code_reference: values.item_code_reference,
              name: values.item_name,
              quantity: parseInt(values.item_quantity),
              discount_rate: FACTURA_DEFAULTS.DEFAULT_FACTURA.items.discount_rate,
              price: parseFloat(values.item_price),
              tax_rate: values.item_tax_rate,
  ...
  const handleView = async (factura: Factura & { key: React.Key }) => {
    ...
    if (factura.number) {
      console.log('Solicitando detalles de factura:', factura.number);
  ...
  const handleDownload = (factura: Factura & { key: React.Key }) => {
    console.log('handleDownload llamado con factura:', factura);
    if (factura.number) {
      console.log('Estableciendo downloadingNumber:', factura.number);
      setDownloadingNumber(factura.number);
    } else {
      console.log('No se encontró number en la factura:', factura);
  ...
  return (
    <Layout >
      <div className="min-h-[80vh] flex flex-col items-center justify-start py-8">
        <div className="w-full max-w-6xl rounded-2xl shadow-xl p-8 border">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <h1 className="text-2xl font-bold text-breanch-900">Gestión de Facturas</h1>
            <Botton className="bg-[#8b4646] text-white font-semibold px-6 py-2 rounded-lg shadow" onClick={handleCreate} texto="Crear Nueva Factura"/>
          </div>
          {loading ? (
            <p className="text-breanch-900">Cargando facturas...</p>
          ) : (
